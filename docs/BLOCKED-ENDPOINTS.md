...
**42 of 336** registered endpoints are **not driven against the live app**.
The rest ARE tested on live: **122** on the default run + **172** via the
...
| Needs setup we lack (business login 403, company logo 500) | 3 |
| Public record write (enquiry / unsubscribe) | 2 |
| **Total not tested on live** | **42** |
...
| Method | Path | Why not tested on live |
| ------ | ---- | ---------------------- |
| `GET` | `/kmail5/v2/readMail/download/{uuid}` | needs a real uploaded attachment (S3 file upload) — the one file-upload gap |
| `GET` | `/kmail5/v2/readMail/downloadThumbnail/{uuid}` | needs a real uploaded attachment (S3 file upload) — the one file-upload gap |
| `GET` | `/kmail5/v2/readMail/mediaStreaming/{uuid}` | needs a real uploaded attachment (S3 file upload) — the one file-upload gap |
| `POST` | `/kmail5/v2/sentMail/getMailCredentials/` | writes state shared by the whole environment (no self-cleaning lifecycle) |
